...
| Windows P3 target | Windows 11 Pro 10.0.26200 x64; Windows PowerShell 5.1.26100.9168; PowerShell 7.6.4 | p3.3-read-only-qualified | The production structured collector passed a live read-only smoke for ActiveStore routes/metrics, stable GUID and hardware identity, DNS and effective NRPT on 2026-08-25. Prior config evidence matched the RedShield adapter and endpoint route; a fresh combined config replay is not run because the external file is absent. Provider handshake/egress and all mutation remain P3.5/P3.4 gates |
