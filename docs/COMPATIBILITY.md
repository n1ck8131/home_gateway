...
| Windows P3 target | Windows 11 Pro 10.0.26200 x64; Windows PowerShell 5.1.26100.9168; PowerShell 7.6.4 | p3.4-offline-mutation-verified | The production structured collector passed a live read-only smoke for ActiveStore routes/metrics, stable GUID and hardware identity, DNS and effective NRPT on 2026-08-25. P3.4 offline ownership, transaction and recovery fault tests pass, but no native mutation backend or live canary has run. Prior config evidence matched the RedShield adapter and endpoint route; a fresh combined config replay is not run because the external file is absent |
