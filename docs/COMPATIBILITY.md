...
| Windows P3 target | Windows 11 Pro 10.0.26200 x64; Windows PowerShell 5.1.26100.9168; PowerShell 7.6.4 | native-offline-verified-field-open | The production collector passed read-only inventory and the native mutation backend is offline-tested. Exact self-hosted Plan/Apply/Confirm, failure recovery and terminal restore remain field gates; the historical RedShield Plan stopped before mutation |
...
| P3 self-hosted Windows client | AmneziaVPN 5.0.1.5 / `7d4f3e0f5090b74903609179653d1f669d2ad08a` | pinned-offline | Windows x64 asset size 91,991,200 and SHA-256 `2e898bbd1d639f5066416961a2a458dba7c3455c0e8f49c7f130e9281d700377` |
| P3 self-hosted AWG | go `v3.1.20260814`; tools/kernel `v3.1.20260812` | pinned-offline | `manifest/versions.lock.yaml` records exact source commits; this is distinct from, and does not replace, the OpenWrt AWG2 baseline |
| P3 server image | `amneziavpn/amneziawg-go:latest` in tagged client Dockerfile | observed-after-install | The rolling server image digest is an operator observation after the separately approved GUI installation, not a design-time pin |
...
| P3 DigitalOcean self-hosted target | Basic Regular $6; `ams3` with `fra1` fallback; Ubuntu 24.04 x64 | gates-6.1-6.3-passed | One owner-approved Droplet and hardened `homegateway` access passed Gates 6.1-6.3. Gate 6.4 needs current read-only reconciliation; Gate 6.5C rolled back exactly one Admin candidate; no protected Guest profile, handshake or egress evidence exists |
