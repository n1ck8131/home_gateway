...
func ImportFile(path, provider string) (Config, error) {
	if provider == "" {
		return Config{}, errors.New("provider ID is required")
	}
	return importFile(path, "", provider)
}
...
func ImportFilePinned(path, expectedSHA256, provider string) (Config, error) {
	...
	return importFile(path, expectedSHA256, provider)
}

func importFile(path, expectedSHA256, provider string) (Config, error) {
