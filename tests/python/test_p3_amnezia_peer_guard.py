...
class PeerGuardProtocolTests(unittest.TestCase):
    ...
    def test_ipv4_policy_preserves_rules_chains_policy_and_order(self):
        original = (
            b"*filter\n:INPUT ACCEPT [10:640]\n:HG-P3-IN - [0:0]\n"
            b"-A INPUT -j HG-P3-IN\n-A HG-P3-IN -j RETURN\nCOMMIT\n"
        )
        normalized = guard._normalize_policy(original)
        changed_counters = original.replace(b"[10:640]", b"[200:12800]")
        self.assertEqual(normalized, guard._normalize_policy(changed_counters))
        for changed in (
            original.replace(b":INPUT ACCEPT", b":INPUT DROP"),
            original.replace(b"HG-P3-IN", b"HG-P3-OTHER"),
            original.replace(b"-j RETURN", b"-j DROP"),
            original.replace(
                b"-A INPUT -j HG-P3-IN\n-A HG-P3-IN -j RETURN",
                b"-A HG-P3-IN -j RETURN\n-A INPUT -j HG-P3-IN",
            ),
            original.replace(b"[10:640]", b"[not-a-counter]"),
        ):
            with self.subTest(changed_sha256=sha(changed)):
                self.assertNotEqual(normalized, guard._normalize_policy(changed))
        with self.assertRaisesRegex(ValueError, "host policy counters differ"):
            guard._normalize_policy(original.replace(b"-A INPUT", b"[1:64] -A INPUT"))

    def test_ipv4_policy_ignores_only_exact_completion_timestamp(self):
        policy = b"*filter\n:INPUT ACCEPT [0:0]\n-A INPUT -j ACCEPT\nCOMMIT\n"
        footer = b"# Completed on Sat Sep  5 14:10:09 2026\n"
        self.assertEqual(guard._normalize_policy(policy + footer), policy)
        self.assertEqual(
            guard._normalize_policy(
                policy + b"# Completed on Mon Jan 31 03:03:38 2011\n"
            ),
            policy,
        )
        for retained in (
            b"# Completed on custom policy\n",
            b" # Completed on Sat Sep  5 14:10:09 2026\n",
            b"# owner policy comment\n",
        ):
            with self.subTest(retained_sha256=sha(retained)):
                self.assertEqual(
                    guard._normalize_policy(policy + retained), policy + retained
                )
        for malformed in (
            b"# Completed on Sat Sep  5 14:10:09 2026 extra\n",
            b"# Completed on Sat Sep  5 24:10:09 2026\n",
            b"# Completed on Sat Sep 32 14:10:09 2026\n",
            b"# Completed on Sat unsupported timestamp\n",
        ):
            with (
                self.subTest(malformed_sha256=sha(malformed)),
                self.assertRaisesRegex(ValueError, "completion timestamp"),
            ):
                guard._normalize_policy(policy + malformed)
        self.assertNotEqual(
            guard._normalize_policy(policy + footer),
            guard._normalize_policy(policy.replace(b"-j ACCEPT", b"-j DROP") + footer),
        )
    ...
    def test_system_collector_uses_independent_persistent_live_metadata_and_temp_sources(
        ...
        outputs[("/usr/sbin/iptables-save",)] = policy
        first = guard.collect_server_snapshot(request, runner=runner)
        outputs[("/usr/sbin/iptables-save",)] = policy.replace(
            b"[10:640]", b"[200:12800]"
        )
        counter_only = guard.collect_server_snapshot(request, runner=runner)
        self.assertEqual(first, counter_only)
        # iptables-save uses "# Completed on <ctime>", without "by".
        footer = b"# Completed on Sat Sep  5 14:10:09 2026\n"
        outputs[("/usr/sbin/iptables-save",)] = policy + footer
        completed_first = guard.collect_server_snapshot(request, runner=runner)
        outputs[("/usr/sbin/iptables-save",)] = policy + footer.replace(
            b"14:10:09", b"14:11:10"
        )
        completed_second = guard.collect_server_snapshot(request, runner=runner)
        self.assertEqual(completed_first, completed_second)
        self.assertEqual(first, completed_first)
        outputs[("/usr/sbin/iptables-save",)] = policy.replace(b"-j RETURN", b"-j DROP")
        changed_rule = guard.collect_server_snapshot(request, runner=runner)
        for key in (
            "host_policy_sha256",
            "firewall_identity_sha256",
            "runtime_identity_sha256",
            ...
            with self.subTest(identity=key):
                self.assertNotEqual(first[key], changed_rule[key])
